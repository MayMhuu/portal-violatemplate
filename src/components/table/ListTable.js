...
import { Table, Pagination, Button, Dropdown, Checkbox, Loader, Icon, Segment, Dimmer } from "semantic-ui-react";
...
const ListTable = ({
  ...
  const listData = useSelector(({ dataTable }) => dataTable.dataList.data || []);
  const total = useSelector(({ dataTable }) => dataTable.dataList.total);
  let sortCol = useSelector(({ dataTable }) => dataTable.column || null);
  let sortDirection = useSelector(({ dataTable }) => dataTable.direction || null);
  ...
  if (columnData.length > 0) {
    columnData.forEach((columnName) => {
      if (!columnName.key.includes("hidden")) {
        fields.push({
          key: columnName.key,
          label: columnName.key === 'actions' ?
            <Dropdown item simple icon={"columns"}>
              <Dropdown.Menu>
                {columns.map(({ key, label }) => (
                  < Dropdown.Item key={key} >
                    <Checkbox size='small' label={variableNameToWords(label)} value={key} defaultChecked={selection.includes(key) ? true : false}
  ...
  useEffect(() => {
    if (currentPage && itemsPerPage) {
      history.push(
        encodeURI(
          `?page=${currentPage}&itemsPerPage=${itemsPerPage}${searchKey ? `&${searchKey}` : ""
          }`,
        ),
      );
    }
  ...
  const defaultRenderBodyRow = (rowIndex, colsWells, wellRow) => {
    ...
    return colsWells.map(({ key }, index) => {
      if (key === 'index') {
        return (<Table.Cell key={index}>  {itemsPerPage && currentPage
          ? rowIndex + 1 + itemsPerPage * (currentPage - 1)
          : rowIndex + 1}</Table.Cell>)
      } else if (key === 'actions') {
        return (
      ...
      return (<Table.Cell key={index}>
        {checkDate ? moment(new Date(wellRow[key])).format("YYYY-MM-DD HH:mm") : wellRow[key]}
      </Table.Cell>)
    });
  }
